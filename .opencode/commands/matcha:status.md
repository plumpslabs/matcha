...
6. **Metrics (if `.agents/matcha-metrics.json` exists)** — planning-gate blocks, shield blocks, reviews run, issues caught, FP rate. Read the JSON directly and report the numbers — this is how matcha proves its own overhead. Zero blocks this session is fine; report what exists.
7. **Health Verdict** — overall session health (Healthy / Needs Attention / Critical)
